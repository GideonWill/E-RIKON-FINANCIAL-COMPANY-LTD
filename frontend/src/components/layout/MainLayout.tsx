...
export const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  // Mobile Horizontal Swipe Gesture to Pull / Dismiss Navigation Drawer
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    const isHorizontalSwipe = Math.abs(deltaX) > Math.abs(deltaY) * 1.1;

    if (isHorizontalSwipe) {
      // Swipe Right -> Pull / Open Navigation Drawer
      if (deltaX > 45 && !isMobileMenuOpen) {
        setIsMobileMenuOpen(true);
      }
      // Swipe Left -> Dismiss / Close Navigation Drawer
      else if (deltaX < -45 && isMobileMenuOpen) {
